...
/*
 * 스프링 부트에서 MVC를 테스트할 때 사용하는 어노테이션이다.()
 */
@WebMvcTest(StudentController.class)
class StudentControllerTest {
	/*
	 * 서버를 구동하지 않고 컨트롤러의 동작을 테스트하기 위한 객체이다.
	 */
	@Autowired
	private MockMvc mockMvc;
	
	/*
	 * @WebMvcTest 어노테이션을 사용하는 경우 테스트 대상 컨트롤러만 빈으로 자동 생성된다.
	 * 따라서 컨트롤러에서 사용하는 서비스 빈은 Mock 객체로 만들어야 한다.
	 * 
	 * @MockBean 어노테이션은 스프링 부트에서 테스트 대상 컨트롤러와 관련된 빈을
	 * Mock 객체로 만들어서 자동으로 주입하는데 사용된다.
	 * 
	 */
	@MockBean //가짜 Bean을 만들어서 주입해주는것
...
//		mockMvc.perform(get("/students").param("deptNo", "001"))
