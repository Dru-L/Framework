...
/*
 * JUnit에서 스프링을 사용할 수 있도록 SpringExtension을 사용하여 확장한다.
...
//	@Qualifier("lee")
	...
	void create() {
		//생성자를 통한 의존성 투입
		// 기존에 자바 애플리케이션에서는 다형성을 통해서 객체간의 결합도를 느슨하게 만들었다.
...
//				new GenericXmlApplicationContext("classpath:spring/root-context.xml");
				...
				//직접 파일의 경로를 지정해서 찾는다.
//				new GenericXmlApplicationContext("file:D:/develop/framework/02_SpringDI/target/classes/spring/root-context.xml");
		
//		Owner owner = (Owner) context.getBean("hong");
//		Owner owner = context.getBean("hong", Owner.class); //클래스 타입으로 객체를 바꿔서 리턴.형변환 방법2
	...
	void annotationApplicationContextTest() {
		ApplicationContext context
			...
			//자바 설정을 읽어서 컨테이너를 만든다.(어플리케이션 컨텍스트)
